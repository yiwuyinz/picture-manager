...
import com.example.picturebackend.manager.FileManager;
...
/**
 * @author Dell
 * @description 针对表【my_picture(图片)】的数据库操作Service实现
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (spaceId != null) {
            ...
            picture.setReviewMessage("个人空间图片");
        }
        //用于判断新增还是更新图片
        Long pictureId = null;
...
//        boolean result = this.saveOrUpdate(picture);
    ...
    public void clearPictureFile(Picture oldPicture) {
        ...
        String key = extractCosPath(oldPicture.getUrl());
        cosManager.deleteObject(key);
        String thumbnailUrl = oldPicture.getThumbnailUrl();
        if (StrUtil.isNotBlank(thumbnailUrl)) {
            thumbnailUrl = extractCosPath(thumbnailUrl);
...
//        checkPictureAuth(loginUser, oldPicture);
    ...
    public static String extractCosPath(String fullUrl) {
        if (StrUtil.isBlank(fullUrl)) {
            return fullUrl;
        }
        try {
            return new java.net.URL(fullUrl).getPath();
        } catch (Exception e) {
            // 降级处理
            return fullUrl.substring(fullUrl.indexOf("/", fullUrl.indexOf("://") + 3));
        }
    }
